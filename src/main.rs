...
#[command(
    name = "sqlserver-mcp",
    about = "SQL Server - master/msdb/sandbox combined catalog MCP server",
    version
)]
struct Cli {
    #[command(subcommand)]
...
#[derive(Subcommand)]
enum Command {
    /// Interactively configure SQL Server - master/msdb/sandbox combined catalog credentials and connection settings
    Setup,
