...
/// another thread at the same time. `tokio::sync::Mutex` rather than
/// `std::sync::Mutex`: an async caller (e.g. a test that awaits
/// `AuthManager::login`, which internally calls `save_credential`) must
/// hold the guard across that `.await`, and holding a `std` mutex guard
/// across an await point is its own bug (`clippy::await_holding_lock`) --
/// sync callers use `.blocking_lock()` instead of `.lock().await`.
#[cfg(test)]
