...
impl McpifyServer {
    ...
    #[tool(
        description = "Semantic search for SQL Server - master/msdb/sandbox combined catalog operations using a natural-language query."
    )]
    async fn search(
        &self,
    ...
    #[tool(
        description = "Return the schema, path, method, and documentation for a specific SQL Server - master/msdb/sandbox combined catalog operationId."
    )]
    ...
    #[tool(
        description = "Validate arguments, invoke a live SQL Server - master/msdb/sandbox combined catalog API operation, and validate the response."
    )]
    async fn call(
        &self,
